    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="container">
            <div class="form-container">
                <h2>Edit Profile MC</h2>
                <form action="{{ route('mc.update', $mc->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        <label for="nama" class="form-label">nama</label>
                        <input type="text" class="form-control" id="nama" name="nama"
                            value="{{ $mc->nama }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="kategori" class="form-label">Kategori</label>
                        <input type="text" class="form-control" id="kategori" name="kategori"
                            value="{{ $mc->kategori }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="kota" class="form-label">Kota</label>
                        <input type="text" class="form-control" id="kota" name="kota"
                            value="{{ $mc->kota }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="deskripsi" class="form-label">Deskripsi</label>
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="5" required>{{ $mc->deskripsi }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="pengalaman" class="form-label">Pengalaman</label>
                        <input type="text" class="form-control" id="pengalaman" name="pengalaman"
                            value="{{ $mc->pengalaman }}" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="media_sosial" name="media_sosial"
                            value="{{ $mc->media_sosial }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="foto" class="form-label">foto Profile</label>
                        <input type="file" class="form-control" id="foto" name="foto">
                        @if ($mc->foto)
                            <img src="{{ asset('storage/' . $mc->foto) }}" alt="Image"
                                style="width: 100px; height: auto;">
                        @endif
                    </div>
                    <button type="submit" class="btn btn-warning btn-sm mb-3 d-inline-flex align-items-center gap-2"
                        style="border-radius: 6px; font-size: 0.85rem; padding: 0.4rem 0.8rem; transition: all 0.2s ease; font-weight: bold;">Simpan
                        Perubahan</button>
                </form>
            </div>
        </div>
